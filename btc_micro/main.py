...
Trades every ~30 seconds with 5x leverage, tight TP/SL, dual long+short.
...
async def run_micro_bot(config: dict, paper_trade: bool = True) -> None:
    ...
    if learner_cfg.get("enabled", True):
        learner = BTCLearner(
            ...
            review_every_n_trades=learner_cfg.get("review_every_n_trades", 15),
            min_trades_to_learn=learner_cfg.get("min_trades_to_learn", 25),
            learning_rate=learner_cfg.get("learning_rate", 0.20),
    ...
    executor = MicroExecutor(
        ...
        position_size_usd=executor_cfg.get("position_size_usd", 50.0),
        leverage=leverage,
        max_consecutive_losses=executor_cfg.get("max_consecutive_losses", 4),
        cooldown_after_losses_sec=executor_cfg.get("cooldown_after_losses_sec", 180),
        ...
        max_trades_per_hour=executor_cfg.get("max_trades_per_hour", 20),
        learner=learner,
    )
    ...
    logger.info("=" * 60)
    logger.info("BTC Micro-Trading Bot Starting")
    ...
    logger.info("  TP: %.2f%% | SL: %.2f%%",
                executor_cfg.get("tp_pct", 0.08), executor_cfg.get("sl_pct", 0.06))
    logger.info("  Position: $%.0f x %dx = $%.0f notional",
                executor_cfg.get("position_size_usd", 50), leverage,
                executor_cfg.get("position_size_usd", 50) * leverage)
    logger.info("  Dual mode: LONG + SHORT simultaneously")
    logger.info("  Max trades/hr: %d", executor_cfg.get("max_trades_per_hour", 20))
    logger.info("  Self-learner: %s", "ENABLED" if learner else "DISABLED")
    if learner:
        logger.info("    Review every: %d trades", learner_cfg.get("review_every_n_trades", 15))
...
def main():
    parser = argparse.ArgumentParser(description="BTC Micro-Trading Bot")
    parser.add_argument(
        "-c", "--config",
